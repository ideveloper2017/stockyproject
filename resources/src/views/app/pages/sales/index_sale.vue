<template>
  <div class="main-content">
    ...
    <div v-else>
      <!-- Summary cards: totals over ALL filtered sales -->
      <b-row class="mb-3">
        <b-col md="4" class="mb-2">
          <b-card body-class="py-3 d-flex align-items-center justify-content-between">
            <div>
              <div class="text-muted small">Обший сумма</div>
              <h4 class="mb-0">{{ totalsAllGrandFormatted }}</h4>
            </div>
        ...
        <b-col md="4" class="mb-2">
          <b-card body-class="py-3 d-flex align-items-center justify-content-between">
            <div>
              <div class="text-muted small">Платный</div>
              <h4 class="mb-0">{{ totalsAllPaidFormatted }}</h4>
            </div>
            <div v-if="totalsLoading" class="spinner sm spinner-primary"></div>
          </b-card>
        </b-col>
        <b-col md="4" class="mb-2">
          <b-card body-class="py-3 d-flex align-items-center justify-content-between">
            <div>
              <div class="text-muted small">Обший долг</div>
              <h4 class="mb-0">{{ totalsAllDueFormatted }}</h4>
            </div>
            <div v-if="totalsLoading" class="spinner sm spinner-primary"></div>
          </b-card>
        </b-col>
...
export default {
  ...
  data() {
    ...
    // Fetch totals for ALL filtered sales (independent of pagination)
      ...
      Sale_id: "",
      limit: "10",
      // Totals across ALL filtered sales
      totalsAllGrand: 0,
      totalsAllPaid: 0,
      totalsAllDue: 0,
      totalsLoading: false,
  ...
  computed: {
    ...
    // Formatted overall totals
    totalsAllGrandFormatted() {
      return ` ${this.formatNumber(this.totalsAllGrand, 2)} ${this.currentUser.currency}`;
    },
    totalsAllPaidFormatted() {
      return `${this.formatNumber(this.totalsAllPaid, 2)} ${this.currentUser.currency}`;
    },
    totalsAllDueFormatted() {
      return `${this.formatNumber(this.totalsAllDue, 2)} ${this.currentUser.currency}`;
    },
  },
  methods: {
