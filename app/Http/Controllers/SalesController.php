    ...
    public function Sale_PDF(Request $request, $id)
    {
        ...
        foreach ($sale_data['details'] as $detail) {
            ...
            if (!empty($detail['product']['image'])) {
                $image_path = public_path('images/products/' . $detail['product']['image']);
                if (file_exists($image_path)) {
                    $image_data = file_get_contents($image_path);
                    $data['image'] = 'data:image/' . pathinfo($image_path, PATHINFO_EXTENSION) . ';base64,' . base64_encode($image_data);
                } else {
                    $data['image'] = null;
                }
            }
